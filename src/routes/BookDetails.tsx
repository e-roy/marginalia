...
export function BookDetails() {
  ...
  // Above the `!book` guard, and the ordering is the whole point: deleting your only
  // book leaves `books.length === 0`, which that guard reads as "still loading" and
  // answers with null — a blank screen that never resolves, because no book is coming.
      ...
      {draft ? (
        ...
        <>
          ...
          */}
          ...
          {/*
            Delete lives here rather than under the notes, where it sat until 2026-08-25.
            Deleting a book is management, not reading — and a destructive control directly
            below a list of notes is both easy to reach by accident and easy to read as
            acting on one of them. `mt-auto` pushes it to the bottom of the screen, well
            clear of Edit.
          */}
          <AlertDialog
            open={confirmOpen}
          ...
          >
            <AlertDialogTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="text-muted-foreground mt-auto self-start"
              >
            ...
            <AlertDialogContent onEscapeKeyDown={(event) => deleting && event.preventDefault()}>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>{cost} This cannot be undone.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
